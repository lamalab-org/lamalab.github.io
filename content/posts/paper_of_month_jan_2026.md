...
This paper addresses the fragility of prompt optimization, showing that prompts tuned on clean inputs often break under realistic noise. The authors introduce PertBench, which evaluates robustness by fixing the instruction prompt and perturbing task inputs with common variations such as typos and paraphrases, revealing significant performance degradation across existing methods. They further propose PGO as a simple baseline that searches for instruction prompts more stable under this evaluation protocol. One aspect I like about this paper is that it narrows down the vast prompt search space into a few interpretable dimensions. That said, I find the optimization procedure somewhat limited, since it explores only specific prompt dimensions and runs for just small iterations, making it unclear whether the prompt variants have truly converged.
